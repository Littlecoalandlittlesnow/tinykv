...
func (r *Raft) sendAppend(to uint64) bool {
	...
	if err != nil {
		...
		if IsEmptySnap(r.RaftLog.pendingSnapshot) {
			...
			if err != nil {
				//log.Info("ID:", r.id, "term:", r.Term, "产生Snapshot失败")
				return false
			}
		} else {
		...
		if debugger {
			log.Info("ID:", r.id, "term:", r.Term, "向ID", to, "发送了一个MsgSnapshot消息，LastIndex:", Snapshot.Metadata.Index, "LastTerm:", Snapshot.Metadata.Term)
		}
		r.msgs = append(r.msgs, pb.Message{
			...
			Term:     r.Term,
			Snapshot: &Snapshot,
			Index:    Snapshot.Metadata.GetIndex(),
			LogTerm:  Snapshot.Metadata.GetTerm(),
		})
		return false
	}
...
func (r *Raft) becomeCandidate() {
	...
	r.votes[r.id] = true
	//候选者多一点时间超时
	r.electionElapsed = 0 - rand.Intn(2*r.electionTimeout)
	r.leadTransferee = None
	if debugger {
			...
			for i, entry := range m.GetEntries() {
				if entry.EntryType == pb.EntryType_EntryConfChange {
					if r.PendingConfIndex <= r.RaftLog.applied {
						...
						if debugger {
							log.Infof("当前ConfChange还没应用，拒绝propose")
						}
						return &util.ErrStaleCommand{}
					}
				}
...
func (r *Raft) handleRequestVote(m pb.Message) {
	...
	if r.Vote == None || r.Vote == m.GetFrom() { //该成员必须还没投票，或者投的就是消息发送者
		...
		if m.GetLogTerm() > logTerm || m.GetLogTerm() == logTerm && m.GetIndex() >= r.RaftLog.LastIndex() {
			//我同意了另一个结点的投票请求意味着我认为他比我更适合当leader，那么我多等一会也没什么关系
			r.electionElapsed -= rand.Intn(2 * r.electionTimeout)
			reject = false
			r.Vote = m.GetFrom()
		}
	}
...
func (r *Raft) handleRequestVoteResponse(m pb.Message) {
	...
	if cntT > len(r.Prs)/2 {
		...
		//设置更多的选举超时时间，让一个已经失败的结点短时间内不会再次发起选举
		r.electionElapsed -= rand.Intn(3 * r.electionTimeout)
	}
}
...
func (r *Raft) handleBeat(m pb.Message) {
	if debugger {
		log.Info("结点", r.id, "term为", r.Term, "正在进行handleBeat")
	}
	r.heartbeatElapsed = 0
	if r.leadTransferee != None {
		r.handleTransferLeader(pb.Message{
	...
	for _, entry := range m.GetEntries() {
		if m.GetIndex() <= r.RaftLog.LastIndex() {
			...
			if Term != entry.GetTerm() {
				//if debugger {
				//	log.Infof("ID：%d进入了entry删除,index:%d", r.id, entry.GetIndex())
				//}
			...
			//if debugger {
			//	log.Infof("ID:%d进入了entry添加,index:%d", r.id, entry.GetIndex())
			//}
			r.RaftLog.entries = append(r.RaftLog.entries, *entry)
		}
	}
...
func (r *Raft) handleAppendEntriesResponse(m pb.Message) {
	...
	r.electionElapsed = 0 - rand.Intn(r.electionTimeout)
	if m.GetReject() == true {
		if r.Prs[m.GetFrom()].Next > m.GetIndex()+1 {
	...
	if m.GetIndex() > r.Prs[m.GetFrom()].Match {
		...
		r.Prs[m.GetFrom()].Next = m.GetIndex() + 1
		//判断是否可以转移领导人
		if r.leadTransferee != None && m.GetFrom() == r.leadTransferee {
			if r.Prs[m.GetFrom()].Match == r.RaftLog.LastIndex() {
				if debugger {
					log.Infof("ID:%d已经拥有最新的日志，具备转移资质", m.GetFrom())
				}
				r.msgs = append(r.msgs, pb.Message{
				...
				if debugger {
					log.Infof("ID:%d还不具备最新的日志，领导人将帮助他获取转移资格", m.GetFrom())
				}
				r.sendAppend(m.GetFrom())
			}
		}
...
func (r *Raft) handleSnapshot(m pb.Message) {
	// Your Code Here (2C).
	if m.GetTerm() < r.Term {
		//回复一个消息让leader早日抛弃幻想，直接变成咸鱼（follower）
		r.sendAppendResponse(m.GetFrom(), true, 0)
		return
	}
	if m.Snapshot.Metadata.GetIndex() <= r.RaftLog.committed {
		return
	}
	if debugger {
		log.Info("ID:", r.id, "term:", r.Term, "正在进行handleSnapshot,来自ID:", m.GetFrom(), "term:", m.GetTerm(), "index:", m.Snapshot.Metadata.Index, "term:", m.Snapshot.Metadata.Term)
	}
	r.becomeFollower(m.GetTerm(), m.GetFrom())
	r.RaftLog.committed = m.Snapshot.Metadata.GetIndex()
	r.RaftLog.applied = m.GetSnapshot().Metadata.GetIndex()
	r.RaftLog.stabled = m.GetSnapshot().Metadata.GetIndex()
	r.RaftLog.HandleSnapshot(m.Snapshot)
	r.Prs = make(map[uint64]*Progress)
	for _, peer := range m.GetSnapshot().Metadata.ConfState.Nodes {
		r.Prs[peer] = &Progress{
			Next:  m.GetIndex() + 1,
			Match: 0,
		}
...
func (r *Raft) handleTransferLeader(m pb.Message) {
	if m.GetFrom() == r.id {
		//if debugger {
		//	log.Infof("转移对象就是自己ID：%d", r.id)
		//}
		return
	}
	to, ok := r.Prs[m.GetFrom()]
	if !ok {
		if debugger {
			log.Infof("不存在该转移对象,ID:%d", m.GetFrom())
		}
		return
	}
	r.leadTransferee = m.GetFrom()
	if to.Match == r.RaftLog.LastIndex() {
		if debugger {
			log.Infof("ID:%d已经拥有最新的日志，具备转移资质", m.GetFrom())
		}
		r.msgs = append(r.msgs, pb.Message{
...
func (r *Raft) addNode(id uint64) {
	// Your Code Here (3A).
	if debugger {
		log.Infof("ID:%d正在进行addNode，添加ID:%d", r.id, id)
	}
	_, ok := r.Prs[id]
	if !ok {
		//为了让领导人直接发快照
		...
		if debugger {
			log.Infof("ID:%d添加ID:%d失败", r.id, id)
		}
	}
}
...
func (r *Raft) removeNode(id uint64) {
	// Your Code Here (3A).
	if debugger {
		log.Infof("ID:%d正在进行removeNode,删除ID:%d", r.id, id)
	}
	_, ok := r.Prs[id]
	if ok {
		delete(r.Prs, id)
		...
		if debugger {
			log.Infof("ID:%d删除ID:%d失败", r.id, id)
		}
		return
	}
	//判断是否有提交的
