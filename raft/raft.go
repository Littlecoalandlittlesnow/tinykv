...
func newRaft(c *Config) *Raft {
	...
	for _, peer := range c.peers {
		if peer == r.id {
			...
			r.Prs[peer] = &Progress{Next: 1, Match: 0}
		}
	}
	return r
}
...
func (r *Raft) sendAppend(to uint64) bool {
	...
	entries := r.RaftLog.getEntriesFromIndex(prevLogIndex + 1)
	r.msgs = append(r.msgs, pb.Message{
...
func (r *Raft) handleRequestVote(m pb.Message) {
	...
	r.Term = m.GetTerm()
	reject := true
	if r.Vote == 0 || r.Vote == m.GetFrom() { //该成员必须还没投票，或者投的就是消息发送者
		//日志更新判断
