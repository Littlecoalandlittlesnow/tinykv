...
func (c *Cluster) CallCommand(request *raft_cmdpb.RaftCmdRequest, timeout time.Duration) (*raft_cmdpb.RaftCmdResponse, *badger.Txn) {
	//if len(request.Requests) > 0 && request.Requests[0].CmdType == raft_cmdpb.CmdType_Put {
	//	log.Infof("key:%d value:%d", request.Requests[0].Put.Key, request.Requests[0].Put.Value)
	//}
	//log.Infof("RegionID:%d PeerID:%d StoreID:%d Term:%d requestLength:%d", request.Header.RegionId, request.Header.Peer.Id, request.Header.Peer.StoreId, request.Header.Term, len(request.Requests))
...
func (c *Cluster) CallCommandOnLeader(request *raft_cmdpb.RaftCmdRequest, timeout time.Duration) (*raft_cmdpb.RaftCmdResponse, *badger.Txn) {
	...
	for {
		...
		//if request != nil && len(request.Requests) > 0 && request.Requests[0].CmdType != raft_cmdpb.CmdType_Get && request.Requests[0].CmdType != raft_cmdpb.CmdType_Snap {
		//	log.Infof("leaderID:%d leaderStoreID:%d requestType:%s", leader.GetId(), leader.StoreId, request.Requests[0].GetCmdType())
		//	log.Infof("RegionID:%d Term:%d", request.Header.RegionId, request.Header.Term)
		//	switch request.Requests[0].CmdType {
		//	case raft_cmdpb.CmdType_Put:
		//		log.Infof("key:%s value:%s", request.Requests[0].Put.Key, request.Requests[0].Put.Value)
		//	case raft_cmdpb.CmdType_Delete:
		//		log.Infof("key:%s", request.Requests[0].Delete.Key)
		//	case raft_cmdpb.CmdType_Get:
		//		log.Infof("key:%s", request.Requests[0].Get.Key)
		//	}
		//}
...
func (c *Cluster) MustPutCF(cf string, key, value []byte) {
	...
	log.Infof("new put %s,%s\n", key, value)
	resp, _ := c.Request(key, []*raft_cmdpb.Request{req}, 5*time.Second)
	if resp.Header.Error != nil {
		panic(resp.Header.Error)
	}
	if len(resp.Responses) != 1 {
		log.Infof("len(resp.Response):%d", len(resp.Responses))
		if resp.AdminResponse != nil {
			log.Infof("adminType:%d", resp.AdminResponse.CmdType)
		}
...
func (c *Cluster) Scan(start, end []byte) [][]byte {
	...
	for (len(end) != 0 && bytes.Compare(key, end) < 0) || (len(key) == 0 && len(end) == 0) {
		log.Infof("new scan request %s-%s\n", key, end)
