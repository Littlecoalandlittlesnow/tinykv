...
func (c *RaftCluster) processRegionHeartbeat(region *core.RegionInfo) error {
	...
	if localRegion != nil {
		if region.GetRegionEpoch().Version < localRegion.GetRegionEpoch().Version || region.GetRegionEpoch().ConfVer < localRegion.GetRegionEpoch().ConfVer {
			return ErrRegionIsStale(region.GetMeta(), localRegion)
		}
	} else {
		localRegions := c.ScanRegions(region.GetStartKey(), region.GetEndKey(), 0)
		for _, value := range localRegions {
			if region.GetRegionEpoch().Version < value.GetRegionEpoch().Version || region.GetRegionEpoch().ConfVer < value.GetRegionEpoch().ConfVer {
				return ErrRegionIsStale(region.GetMeta(), value.GetMeta())
			}
		}
	}
	c.putRegion(region)
